...
echo ""
echo -e "${dim}  exemplos de uso (12 tools disponiveis):${reset}"
echo -e "${dim}    > 'liste meus workspaces'                  → cockpit_list_workspaces${reset}"
echo -e "${dim}    > 'mostre o card SW78'                     → cockpit_show_card${reset}"
echo -e "${dim}    > 'crie um card pra refatorar auth'        → cockpit_create_card${reset}"
echo -e "${dim}    > 'descarte o SW82, decidi nao fazer'      → cockpit_archive_card${reset}"
...
echo -e "${dim}    > 'implementa o SW79'                      → cockpit_implement_async${reset}"
echo -e "${dim}    > 'implementa SW79 e SW80 em paralelo'     → ...async (isolation: worktree)${reset}"
echo -e "${dim}    > 'como ta a session sess_xyz?'            → cockpit_get_session${reset}"
echo ""
